...
public class Main{
    ...
    public static void main(String[] args) {
        ...
        System.out.println("------------------");
        System.out.println(pos + ". position: " + randomizedSelect(a, 0, n - 1, pos, n, displayTemporaryArrays, size));
        printArray(aCopy, "i");
        printArray(a, "f");
        insertionSort(a);
        printArray(a, "s");
    ...
    public static int findMedianOfMedians(ArrayList<Integer> a, int p, int q, boolean displayTemporaryArrays, int size) {
        if (q - p < size) {
        ...
        for (int i = p; i <= q; i += size) {
            int end = Math.min(i + size - 1, q);
            ArrayList<Integer> group = new ArrayList<>(a.subList(i, end + 1));
            insertionSort(group);
            medians.add(group.get((group.size() - 1) / 2));
        }
        return randomizedSelect(medians, 0, medians.size() - 1, (medians.size() + 1) / 2, medians.size(), displayTemporaryArrays, size);
    }
    ...
    public static int randomizedSelect(ArrayList<Integer> a, int p, int q, int i, int n, boolean displayTemporaryArrays, int size) {
        ...
        int pivotValue = findMedianOfMedians(a, p, q, displayTemporaryArrays, size);
        ...
        if (i == k) {
            ...
            return randomizedSelect(a, p, pivotIndex - 1, i, n, displayTemporaryArrays, size);
        } else {
            return randomizedSelect(a, pivotIndex + 1, q, i - k, n, displayTemporaryArrays, size);
        }
    }
